...
| `gpuIds` | GPU pool specification | Only if `runsOn` is `"GPU"` | Comma-separated pool IDs (e.g., `"ADA_24"`) with optional GPU ID negations (e.g., `"-NVIDIA RTX 4090"`). Available pool IDs are [listed below](#gpu-pools). For a list of GPU IDs to use for negations, see [GPU Types](/references/gpu-types).  |
...
This table contains a list of valid GPU pools that you can specify in the `gpuIds` field. Use this to define which GPUs can be used by workers on a deployed instance of your repo.
