...
| `gpuIds` | GPU pool specification | Only if `runsOn` is `"GPU"` | Comma-separated pool IDs (e.g., `"ADA_24"`) with optional GPU ID negations (e.g., `"-NVIDIA RTX 4090"`). Available pool IDs are [listed below](#gpu-pools). For a list of GPU IDs you can use for negations, see [GPU Types](/references/gpu-types).  |
...
Use the pool IDs below to specify which GPUs can be used by workers after the endpoint is deployed.
