...
| **FLUX.1 Kontext [dev]** | A 12 billion parameter rectified flow transformer capable of editing images based on text instructions. | `https://api.runpod.ai/v2/black-forest-labs-flux-1-kontext-dev/` |
| **Deep Cogito v2 Llama 70B** | Open-source hybrid reasoning language model. | `https://api.runpod.ai/v2/deep-cogito-v2-llama-70b/` |
...
| **Qwen Image** | Image generation foundation model with advanced text rendering. | `https://api.runpod.ai/v2/qwen-image-t2i/` |
| **Qwen Image LoRA** | Image generation with LoRA support and advanced text rendering. | `https://api.runpod.ai/v2/qwen-image-t2i-lora/` |
| **Qwen Image Edit** | Image editing with unique text rendering capabilities. | `https://api.runpod.ai/v2/qwen-image-edit/` |
| **Seedream 4.0 T2I** | New-generation image creation with unified generation and editing architecture. | `https://api.runpod.ai/v2/seedream-v4-t2i/` |
| **Seedream 4.0 Edit** | New-generation image editing with unified generation and editing architecture. | `https://api.runpod.ai/v2/seedream-v4-edit/` |
| **Seedream 3.0** | Native high-resolution bilingual image generation (Chinese-English). | `https://api.runpod.ai/v2/seedream-3-0-t2i/` |
| **Seedance 1.0 Pro** | High-performance video generation with multi-shot storytelling. | `https://api.runpod.ai/v2/seedance-1-0-pro/` |
| **Nano Banana Edit** | Google's state-of-the-art image editing model. | `https://api.runpod.ai/v2/nano-banana-edit/` |
| **WAN 2.5** | Image-to-video generation model. | `https://api.runpod.ai/v2/wan-2-5/` |
| **WAN 2.2 I2V 720p LoRA** | Open-source video generation with LoRA support. | `https://api.runpod.ai/v2/wan-2-2-t2v-720-lora/` |
| **WAN 2.2 I2V 720p** | Open-source AI video generation model that uses a diffusion transformer architecture for image-to-video generation. | `https://api.runpod.ai/v2/wan-2-2-i2v-720/` |
| **WAN 2.2 T2V 720p** | Open-source AI video generation model that uses a diffusion transformer architecture for text-to-video generation. | `https://api.runpod.ai/v2/wan-2-2-t2v-720/` |
| **WAN 2.1 I2V 720p** | Open-source AI video generation model that uses a diffusion transformer architecture for image-to-video generation. | `https://api.runpod.ai/v2/wan-2-1-i2v-720/` |
| **WAN 2.1 T2V 720p** | Open-source AI video generation model that uses a diffusion transformer architecture for text-to-video generation. | `https://api.runpod.ai/v2/wan-2-1-t2v-720/` |
| **Kling v2.1 I2V Pro** | Professional-grade image-to-video with enhanced visual fidelity. | `https://api.runpod.ai/v2/kling-v2-1-i2v-pro/` |
| **Whisper V3 Large** | State-of-the-art automatic speech recognition. | `https://api.runpod.ai/v2/whisper-v3-large/` |
| **Minimax Speech 02 HD** | High-definition text-to-speech model. | `https://api.runpod.ai/v2/minimax-speech-02-hd/` |
...
### Deep Cogito v2 Llama 70B

Deep Cogito v2 Llama 70B is an open-source hybrid reasoning language model with advanced reasoning capabilities.
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `prompt` | string | Yes | - | Input text or question for the model. |
| `sampling_params` | object | No | {} | Sampling configuration parameters. |
| `sampling_params.max_tokens` | integer | No | 1024 | Maximum number of tokens to generate. |
| `sampling_params.temperature` | float | No | 0.7 | Sampling temperature (higher = more random). |
| `sampling_params.seed` | integer | No | -1 | Seed for reproducible results. -1 generates a random seed. |
| `sampling_params.top_k` | integer | No | -1 | Top-k sampling parameter. |
| `sampling_params.top_p` | float | No | 1 | Top-p (nucleus) sampling parameter. |
