...
You can make API requests to public endpoints using any HTTP client. The endpoint URL is specific to the model you want to use. For example, for Flux Dev the endpoint is `https://api.runpod.ai/v2/black-forest-labs-flux-1-dev/run`.
...
Depending on the model, each endpoint accepts a different set of parameters to control the generation process.
