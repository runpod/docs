...
The dataset type is set to `null` by default. Depending on the dataset you selected, you will need to change this. 

For example, if you selected the `mhenrichsen/alpaca_2k_test` dataset, you'll need to change `type: null` to `type: alpaca` to load the dataset.

You can find an overview of common dataset types below:
