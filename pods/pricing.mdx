...
You commit to a usage term, such as 3 months or 6 months, by making an upfront payment. During this term, you receive a considerably lower hourly rate for your Pod.
...
- Fault-tolerant workloads that can withstand interruptions (e.g., batch processing, data analysis, rendering, simulations, CI/CD tasks).
...
RunPod provides [two types of storage](/pods/storage/types):

*   **Pod storage (disk volumes):** This is the default storage type for Pods. It is billed at \$0.10 per GB per month for storage on running Pods and \$0.20 per GB per month for volume storage on stopped Pods. This storage is tied to compute servers.
*   **Network volumes:** This is an external storage type that is billed at \$0.07 per GB per month for storage requirements below 1TB. For requirements exceeding 1TB, the rate is \$0.05 per GB per month. Network volumes are billed on a per-hour basis.

Storage billing is also pro-rated per minute. You are not charged for storage if the host machine is down or unavailable from the public internet.
...
4.  Review your Pod's configuration details, including the terms of the selected pricing model (e.g., upfront cost and duration for savings plans, or the interruptible nature of Spot instances).
...
*   For **maximum flexibility and reliability** for short-term or unpredictable workloads, choose **on-demand**.
*   For **significant cost savings on long-term, stable workloads**, and if you can make an upfront commitment, choose a **savings plan**.
*   For the **lowest possible cost on fault-tolerant, interruptible workloads**, choose **spot instances**.
