...
<Tabs>
  ...
  <Tab title="VSCode">
   ...
   3. Copy the second command, under **SSH over exposed TCP**.

      <Warning>
      If you only see one command under SSH, then SSH over exposed TCP is not supported by your selected Pod template (i.e., the template does not start an SSH service). That means you won't be able to connect to your Pod directly through VSCode/Cursor, but you can still connect using [basic SSH](/pods/connect-to-a-pod#basic-ssh-connection) via the terminal.
      </Warning>
...
3. Under **Direct TCP Ports**, look for a line similar to:
   ...
   <Warning>
   If you don't see a **Direct TCP Ports** section, then direct TCP ports are not supported by your selected Pod template (i.e., the template does not start an SSH service). That means you won't be able to connect to your Pod directly through VSCode/Cursor, but you can still connect using [basic SSH](/pods/connect-to-a-pod#basic-ssh-connection) via the terminal.
   </Warning>
