...
To connect to a Pod, you'll need to install the Remote-SSH extension for your IDE:
   ...
   2. Search for and install the Remote-SSH extension:
      - VSCode: **Remote - SSH: Editing Configuration Files** by **ms-vscode-remote**.
...
<Warning>
If you've already deployed a Pod before generating an SSH key, you'll need to add the key to your Pod's [environment variables](/pods/references/environment-variables) using the `PUBLIC_KEY` variable. If you deploy a Pod after generating an SSH key, it will be added automatically.
</Warning>
