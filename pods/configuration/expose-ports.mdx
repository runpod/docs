...
If you're having issues with port exposure:

- **Service not accessible via proxy**: Ensure your service binds to `0.0.0.0` (all interfaces) not just `localhost` or `127.0.0.1`.
- **524 timeout errors**: Your service takes longer than 100 seconds to respond. Consider using TCP or restructuring your application for faster responses.
