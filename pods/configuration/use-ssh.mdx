...
## Generate your SSH key and add it to your Runpod account
...
1.  Ensure you have an [SSH key pair](#generate-your-ssh-key-and-add-it-to-your-runpod-account) generated on your local machine and added to your Runpod account.
...
If you are using a Runpod official template, such as Runpod PyTorch or Stable Diffusion, no additional setup is required, as full SSH access is already configured for you.

However, if you're using a custom template, ensure that TCP port 22 is exposed and that the SSH daemon is running inside your Pod. To do this, add the following Docker command to your template. Or, if you already have a custom start command, replace `sleep infinity` at the end with your existing command with this:
