...
<Tip>
If your Pod has a [network volume](/pods/storage/create-network-volumes) attached, it cannot be stopped, only terminated. When you terminate the Pod, your data in the `/workspace` directory will be preserved in the network volume, and you can regain access by deploying a new Pod with the same network volume attached.

To learn more about storage options, see [Storage overview](/pods/storage/types).
</Tip>
