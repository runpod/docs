...
Access logs from the [Pods page](https://www.console.runpod.io/pods) by expanding your Pod and clicking **Logs**. You can also read the same container and system logs from the terminal with [`runpodctl pod logs`](/runpodctl/reference/runpodctl-pod), which streams them as JSON Lines for piping and scripting.
