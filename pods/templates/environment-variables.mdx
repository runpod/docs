...
| Variable | Description |
| --- | --- |
| `RUNPOD_ALLOW_IP` | Comma-separated list of IP addresses or CIDR ranges allowed to reach your Pod's HTTP proxy ports (for example, `203.0.113.7,198.51.100.0/24`). Requests from other addresses receive a `400` response. Does not apply to TCP ports. See [Restrict access by IP address](/pods/configuration/expose-ports#restrict-access-by-ip-address). |
