...
- **Never hardcode secrets**: Use environment variables or Runpod Secrets for sensitive data.
- **Use descriptive names**: Choose clear, descriptive variable names like `DATABASE_PASSWORD` instead of `DB_PASS`.
- **Validate required variables**: Check that critical environment variables are set before your application starts.
...
- **Group related variables**: Use consistent prefixes for related configuration (e.g., `DB_HOST`, `DB_PORT`, `DB_NAME`).

**Application design**:

- **Fail fast**: If required environment variables are missing, exit early with clear error messages.
- **Type conversion**: Convert string environment variables to appropriate types (integers, booleans) in your application.
