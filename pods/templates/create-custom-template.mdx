...
<Warning>
If you're pre-load a model with `wget` (option 2 from step 5), you need to uncomment the `classifier = pipeline()` object in `main.py` and comment out the `classifier = pipeline()` object for option 1.
</Warning>
