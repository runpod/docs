...
Pods provide instant access to powerful GPU and CPU resources for AI <TrainingTooltip />, <FineTuningTooltip />, rendering, and other compute-intensive workloads.
...
- [Storage](#storage-options):
    ...
    - <VolumeDiskTooltip />: Persistent storage that is preserved between Pod starts and stops.
    - <NetworkVolumeTooltip />: Permanent, portable storage that can be moved between <MachinesTooltip /> and persists even after Pod deletion.
...
Every Pod comes with a resizable <PodContainerDiskTooltip /> that houses the operating system and stores temporary files, which are cleared after the Pod stops.

By contrast, <VolumeDiskTooltip />s provide persistent storage that is preserved throughout the Pod's lease, functioning like a dedicated hard drive. Data stored in the volume disk directory (`/workspace` by default) persists when you stop the Pod, but is erased when the Pod is deleted. 
...
* [Set up Ollama on a Pod](/tutorials/pods/run-ollama) to run LLM <InferenceTooltip /> with HTTP API access.
