...
- **Disk volume**: Permanent storage preserved for the duration of your Pod's lease (i.e., until you terminate the Pod).
- **Network volume (optional)**: Portable storage that can be moved between machines and persists even after Pod deletion. When used, this replaces the disk volume.
...
Pods offer three [storage types](/pods/storage/types) to match different use cases:
...
**Disk volumes** provide permanent storage that is preserved throughout the Pod's lease, functioning like a dedicated hard drive. These are cleared only after the Pod is deleted.

Optional **network volumes** provide flexible storage that can be transferred between machines, replacing the disk volume when attached. When using network storage, you can safely delete your Pod without losing the data stored in your network volume directory (`/workspace` by default).
