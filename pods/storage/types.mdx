...
4. Adjust the storage capacity for the container or volume disk. Volume disk size can be increased, but not decreased.
5. Click **Save** to apply the changes.

<Warning>

Editing a running Pod will cause it to reset completely, erasing all data that isn't stored in your volume disk/network volume mount path (`/workspace` by default). 
