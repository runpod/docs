...
description: "Persistent, portable storage for Pods and Serverless workers."
---

Network volumes offer persistent storage that exists independently of the lifecycle of a Pod or Serverless worker. This means your data is retained even if a Pod is terminated or a worker is stopped. They can be attached to multiple Pods (though not simultaneously to multiple running Pods), making them ideal for sharing data, transferring storage between machines, or maintaining datasets for Serverless workers.
...
*   **Shareable storage:** Use the same data across different Pods or between multiple Serverless workers. While a network volume can be *attached* to multiple Pod configurations, it can only be *actively used* by one running Pod or Serverless worker at a time to prevent data corruption. You should typically stop a Pod before attaching its network volume to a new Pod.
...
Network volume storage space costs significantly less than for disk volumes (\$0.07/GB/month rather than \$0.10/GB/month), and storing data on a network volume can be more economical than provisioning separate disk space for multiple Pods, even with just two Pods sharing one volume.
