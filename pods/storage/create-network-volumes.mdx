...
<Tab title="REST API">
To create a network volume using the REST API, send a POST request to the `/networkvolumes` endpoint. You can also list existing network volumes using a GET request:
...
For complete API documentation and parameter details, see the [List network volumes API reference](/api-reference/network-volumes/GET/networkvolumes).
