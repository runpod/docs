...
description: "Learn how to create and configure endpoint functions with the Flash Endpoint class."
---

import { WorkerTooltip, ServerlessTooltip, NetworkVolumesTooltip } from "/snippets/tooltips.jsx";

The `Endpoint` class is the single entry point for all Flash functionality. It marks Python functions for execution on Runpod's <ServerlessTooltip /> infrastructure, handling resource provisioning, dependency installation, and data transfer automatically.

## Basic usage

The simplest way to use Flash is with the `@Endpoint` decorator:
...
When you call this function, Flash:
1. Provisions a GPU worker on Runpod (or reuses an existing one).
2. Installs the specified dependencies.
3. Executes your function on the remote GPU.
4. Returns the result to your local environment.
