---
title: "Create Flash API endpoints"
sidebarTitle: "Create API endpoints"
...
Unlike standalone scripts that run once and return results, API endpoints create a persistent server that handles incoming HTTP requests. Each request is processed by a Serverless worker using the same remote functions you'd use in a standalone script.
