...
**Cause:** Flash requires Python 3.11 or 3.12.
...
Alternatively, use a Docker container with Python 3.11 or 3.12 for your build environment.
...
**Symptom:** Model fails to load with a security warning about remote code execution.

**Cause:** Some Hugging Face models (like RMBG, certain vision transformers, and custom architectures) require `trust_remote_code=True` to load custom model code from the repository.

**Solution:** Pass `trust_remote_code=True` when loading the model:
...
<Warning>
Only use `trust_remote_code=True` with models from trusted sources. This setting allows the model repository to execute arbitrary Python code on your worker.
</Warning>

### Common ML library compatibility issues

When using popular ML libraries together, you may encounter compatibility issues. Here are some tested combinations:
...
dependencies=[
    "torch>=2.0.0,<2.5",
    "transformers>=4.35.0,<4.46",
    "diffusers>=0.25.0",
    "accelerate>=0.25.0"
]
```

**Segmentation models (like RMBG, SAM):**
...
dependencies=[
    ...
    "transformers>=4.35.0",
    "pillow>=10.0.0",
    "timm>=0.9.0"  # Often required for vision models
]
```
...
<Tip>
If you encounter dependency conflicts, try pinning to specific versions that are known to work together. Check the library documentation or GitHub issues for compatible version combinations.
</Tip>

### Worker import errors

**Symptom:** Function raises `NameError: name 'helper_function' is not defined` or similar.

**Cause:** Helper functions, classes, or variables defined outside the `@Endpoint` function are not available on the remote worker. Flash serializes only the decorated function body, not external code.

**Solution:** Make your endpoint function self-contained. See [Serialization boundaries](/flash/execution-model#serialization-boundaries) for details.
...
def compute(data):
    return process_data(data)  # Error: process_data not defined

# Correct - helper inside the function
@Endpoint(name="worker", gpu=GpuGroup.ANY)
def compute(data):
    def process_data(x):
        return x * 2
