...
**Cause:** An `@Endpoint` file imports a local (non-pip) module that Flash can't resolve to a bundle-able file, such as a broken relative import, a missing package submodule, or a file outside the project root. Flash fails the build instead of shipping an artifact that would break on the worker.
