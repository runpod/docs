...
config = LiveServerless(
    ...
    idleTimeout=60  # Seconds before idle workers scale down
)
```
...
<Note>
When using `flash build` or `flash deploy`, dependencies are pre-installed in the worker image, eliminating pip installation at request time. When running standalone scripts with `@remote` functions outside of a Flash app, dependencies may be installed on the worker at request time.
</Note>
