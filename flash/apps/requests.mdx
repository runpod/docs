...
<Warning>
The `input` object must contain at least one field. Empty input (`{"input": {}}`) or missing input is rejected with a 422 error. If your function has optional parameters, pass explicit values or `null` for them:

```json
{"input": {"optional_param": null}}
```
</Warning>
...
| `404` | Route not found |
| `422` | Unprocessable entity (empty or null input) |
