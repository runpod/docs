...
- [Load balancing endpoints](/flash/endpoint-functions#load-balanced-endpoints): Create HTTP APIs with custom routes using `.get()`, `.post()`, etc. decorators.
- [Queue-based endpoints](/flash/endpoint-functions#queue-based-endpoints): Run compute tasks using the `/runsync` or `/run` routes.
...
Which packages to exclude depends on your endpoint configuration:

| Configuration type | Base image | Pre-installed packages |
|--------------|------------|------------------------|
| GPU (`gpu=`) | PyTorch base | `torch`, `torchvision`, `torchaudio` |
| CPU (`cpu=`) | Python slim | None |
