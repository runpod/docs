...
- [Load balancing endpoints](/flash/configuration/load-balancing): Create HTTP APIs with custom routes using `@remote(method="POST", path="/...")`.
- [Queue-based endpoints](/flash/configuration/queue-based): Run compute tasks using the `/runsync` or `/run` routes.
