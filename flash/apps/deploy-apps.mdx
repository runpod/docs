...
gpu_config_1 = LiveServerless(name="run-model", gpus=[GpuGroup.AMPERE_80])
gpu_config_2 = LiveServerless(name="preprocess", gpus=[GpuGroup.AMPERE_80])
...
- `https://api-abc123.runpod.net` (run-model)
- `https://api-def456.runpod.net` (preprocess)
...
This creates:
- **One Serverless endpoint**: `https://api-abc123.runpod.net` (named "api")
