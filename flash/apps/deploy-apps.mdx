...
<Note>
Cross-endpoint calls also work during local development. When you run your app with `flash dev` (or its `flash run` alias), the endpoint handling each request carries the API token it needs to reach its peers, so you can test the full pipeline pattern locally before deploying to production.
</Note>
