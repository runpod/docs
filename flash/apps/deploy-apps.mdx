...
- [Load balancing endpoints](/flash/configuration/overview#load-balancing-endpoints): Create HTTP APIs with custom routes using `@remote(method="POST", path="/...")`.
- [Queue-based endpoints](/flash/configuration/overview#queue-based-endpoints): Run compute tasks using the `/runsync` or `/run` routes.
