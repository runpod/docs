...
await job.wait(timeout=60)   # Wait for completion (retries transient network errors)
await job.cancel()           # Cancel the job
```

The `wait()` method automatically retries transient network errors with exponential backoff, making it resilient during cold starts. HTTP status errors like 401 and 404 are not retried.
