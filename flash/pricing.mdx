...
3. **Idle time**: The period a worker remains active after completing a request, waiting for additional requests before scaling down. You're charged for idle time at the same rate as during active compute time.
