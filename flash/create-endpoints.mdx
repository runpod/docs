...
You must import pip/installed packages **inside the decorated function body**, not at the top of your file. This ensures imports happen on the remote worker. Import local project modules inside the function body too. See [Import local modules](#import-local-modules) for details.
...
Put local imports inside the decorated function body. When you run an endpoint live, by calling it directly or during `flash dev`, Flash ships only the function's source. It scans only that source for local imports. Imports at the top of the file aren't shipped or run on the worker, and the function fails when it uses them. The same code works unchanged with `flash deploy`, which bundles whole files.

```python
from runpod_flash import CpuInstanceType, Endpoint

@Endpoint(name="greeter", cpu=CpuInstanceType.CPU3C_1_2)
async def greet(name: str = "world", lang: str = "en") -> dict:
    # Local modules that sit next to this file
    import text_utils             # sibling module: text_utils.py
    from greetings import render  # local package: greetings/__init__.py

    return {"greeting": text_utils.shout(render(name, lang))}
```

For the complete project, see the [local modules example](https://github.com/runpod/flash-examples/tree/main/01_getting_started/05_local_modules) in the Flash examples repository.

Flash supports absolute imports (`import utils`), relative imports (`from . import helpers`), and dynamic imports with a literal name (`importlib.import_module("plugin")`). It also pulls in the `__init__.py` files for any packages you import. Flash can't resolve dynamic imports whose module name is computed at runtime, so it emits a warning, and you're responsible for making those modules available on the worker.
