...
You can also run Flash through its [Nix](https://nixos.org) flake, which bundles its own Python and dependencies. Run the CLI straight from GitHub without cloning the repository or installing Python:
...
This path requires Nix with flakes enabled. For installing Nix, enabling flakes, and pinning a specific version, see the [Flash flake guide](https://github.com/runpod/flash/blob/main/nix/README.md).
