...
The `wait()` method automatically retries transient network errors (connection drops, timeouts, protocol errors) with exponential backoff, making it resilient during cold starts while workers initialize. After five consecutive polling errors, the underlying network error is raised. HTTP status errors like 401 (authentication) or 404 (not found) fail immediately without retry.
