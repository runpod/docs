...
When `max_concurrency` is greater than 1, Flash configures the worker to accept multiple jobs at once. This is useful for I/O-bound workloads where a worker spends time waiting for external resources like API calls, database queries, or file downloads.

<Warning>
Concurrent execution requires async handlers. If you set `max_concurrency > 1` on a sync handler, Flash logs a warning. For best results, make your handler function `async` and use `await` for I/O operations.
</Warning>
