...
| `max_concurrency` | `int` | Maximum concurrent jobs per worker (queue-based only). Values >1 only achieve true concurrency with async handlers; sync handlers log a build-time warning. | `1` |
...
Maximum number of jobs a single worker can process concurrently. This parameter applies only to queue-based endpoints and is ignored for load-balanced endpoints.
...
`max_concurrency` controls how many jobs a single worker processes simultaneously. Each worker pulls up to `max_concurrency` jobs from the queue at once. This is useful for I/O-bound workloads where a worker spends time waiting for external resources like API calls, database queries, or file downloads.

<Warning>
True concurrent execution requires async handlers. With sync handlers, the runtime pulls multiple jobs from the queue but processes them one at a time—and Flash logs a build-time warning. For best results, make your handler function `async` and use `await` for I/O operations.
</Warning>

**Recommendations**:
- `max_concurrency=1` (default): Use for GPU-bound workloads where each job fully utilizes the GPU.
- `max_concurrency=2-8`: Use for mixed workloads with some I/O wait time.
- `max_concurrency>8`: Use for highly I/O-bound workloads. Ensure your handler and GPU can handle this level of concurrency.

**Validation and warnings**:
- `max_concurrency` must be at least 1. Values less than 1 raise a validation error.
- Values greater than 100 log a capacity warning at build time, since most GPU workloads saturate well below this threshold.

<Note>
Load-balanced endpoints handle concurrency through their underlying web server and do not use this parameter. If you set `max_concurrency` on a load-balanced endpoint, it is ignored and Flash logs a warning.
</Note>
