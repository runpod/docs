...
@Endpoint(
    ...
    idle_timeout=20,        # Keep workers longer for variable traffic
...
- `idle_timeout`: 15-30 minutes for production workloads.
...
api = Endpoint(
    ...
    idle_timeout=30,        # Keep workers active longer
...
- `idle_timeout`: 20-30 minutes to keep workers ready.
...
- **Use short `idle_timeout`** (5 minutes) to scale down quickly between test runs.
...
@Endpoint(
    ...
    idle_timeout=5          # Quick scale-down
)
...
- **Reduce `idle_timeout`** for sporadic workloads (5-10 minutes).
...
@Endpoint(
    ...
    idle_timeout=5,         # Fast scale-down
