...
Flash's `LiveServerless` and `LiveLoadBalancer` configurations handle most use cases, allowing you to execute arbitrary Python code remotely without managing Docker images yourself. 

However, for specialized environments that require custom Docker images (such as pre-built ML frameworks, or workers that require specific CUDA versions) you can use `ServerlessEndpoint` or `CpuServerlessEndpoint` to deploy your own custom Docker images.
...
| Framework | Image name | Image link |
|-----------|-------|---------------|
...
1. [Build a handler function](/serverless/workers/handler-functions) that will be used to process requests.
2. [Create a Dockerfile](/serverless/workers/create-dockerfile) that will be used to build the Docker image.
3. [Push the Docker image to a registry](/serverless/workers/deploy).
4. Reference the image in `imageName` when creating the `ServerlessEndpoint` or `CpuServerlessEndpoint` configuration.

## Deploy a custom image

After you've created a custom image using the steps above, you can use the `ServerlessEndpoint` or `CpuServerlessEndpoint` configuration to deploy it and make requests to the endpoint using the `ResourceManager` class:
...
```python
from runpod_flash import ServerlessEndpoint, GpuType

config = ServerlessEndpoint(
    name="my-custom-worker",
    imageName="your-registry/your-image:tag",
    gpus=[GpuType.NVIDIA_GEFORCE_RTX_4090],
    workersMax=3
)
```
</Step>
...
Expand the section below to see a complete example of deploying a vLLM endpoint using a custom Docker image:

<Accordion title="vLLM inference example">

This script uses Runpod's official [vLLM worker](/serverless/vllm/overview) to deploy the `microsoft/Phi-3.5-mini-instruct` language model:

```python"
import asyncio
from runpod_flash import ServerlessEndpoint, GpuType, ResourceManager

# Configure vLLM endpoint
vllm_config = ServerlessEndpoint(
    name="vllm-small-model",
    imageName="runpod/worker-vllm:stable-cuda12.1.0",
    gpus=[GpuType.NVIDIA_GEFORCE_RTX_4090],  # RTX 4090 24GB
    workersMax=3,
    env={
...
See the [resource configuration reference](/flash/configuration/parameters) for all available options.
...
Pass configuration to your Docker image via the `env` parameter:

```python
config = ServerlessEndpoint(
    name="vllm-worker",
    imageName="runpod/worker-vllm:stable-cuda12.1.0",
    env={"MODEL_NAME": "meta-llama/Llama-3.2-3B-Instruct", "HF_TOKEN": "hf_..."}
)
```

See [Configuration parameters](/flash/configuration/parameters#env) for details on how environment variables work with Flash configurations.
...
- Use a larger GPU: `gpus=[GpuType.NVIDIA_A100_80GB_PCIe]`
...
- [View the resource configuration reference](/flash/configuration/parameters) for all `ServerlessEndpoint` options.
