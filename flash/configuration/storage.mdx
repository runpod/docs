...
### How Flash resolves volumes by name or ID

When you reference a volume by `name`, Flash treats the name as the source of truth. It resolves the name to a volume on Runpod, creating one if no match exists, and caches the resolved volume ID in `.flash/resources.pkl`. Flash revalidates that cached ID against Runpod on each deploy. If the volume was deleted outside of Flash (through the console or API), Flash re-resolves the name and recreates the volume automatically, so your next deploy keeps working instead of failing.

A volume referenced only by `id` has no name to fall back on, so if it's deleted out of band, Flash can't recreate it and reports a clear error. Reference volumes by `name` whenever you want Flash to manage their lifecycle for you.
