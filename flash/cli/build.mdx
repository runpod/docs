...
### Local modules are force-included

Local (non-pip) modules that your endpoints import are always bundled, even when an ignore pattern would otherwise exclude them. During the build, Flash resolves the import closure of each `@Endpoint` file and force-includes the files those imports depend on. For example, if an endpoint imports a sibling `test_helpers.py`, Flash bundles it even though the `test_*.py` ignore rule would normally drop it.

If an `@Endpoint` file imports a local module that Flash can't resolve (such as a broken relative import or a file outside the project root), the build fails with a clear error instead of shipping a broken artifact. A file that fails resolution but doesn't define an endpoint is skipped with a warning. For details on how local imports are resolved and bundled, see [Import local modules](/flash/create-endpoints#import-local-modules).
