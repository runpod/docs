...
When you run `flash build` or `flash deploy`, the build prints the resolved Python version and its source, so you can confirm which version your workers will target before deploying. The source is one of `matched local interpreter`, `--python-version override`, or `declared on resource <name>`. For example, running Flash from a Python 3.11 interpreter with no override prints `targeting Python 3.11 (matched local interpreter)`.
