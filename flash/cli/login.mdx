...
1. `RUNPOD_API_KEY` environment variable (highest priority).
2. `RUNPOD_API_KEY` in a `.env` file (loaded automatically at startup).
3. Credentials file (`~/.runpod/config.toml`) (lowest priority).
...
### Forcing environment-variable-only authentication

If you want to ensure Flash only uses a specific API key and ignores any stored credentials, set the environment variable explicitly in your script or shell:

```bash
# In your shell - ensures this key is used regardless of stored credentials
export RUNPOD_API_KEY="your_api_key"
python your_script.py
```

Or in Python before importing Flash:

```python
import os
os.environ["RUNPOD_API_KEY"] = "your_api_key"

# Now import and use Flash
from runpod_flash import Endpoint, GpuGroup
```

This is useful when:

- You need to use different API keys for different projects.
- You want to override credentials in CI/CD pipelines.
