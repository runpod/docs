...
Each datacenter should also have a CPU server which provides CPU-only pods and serverless workers. Additionally, Runpod will use this server to host additional features for which a GPU is not required (ex. S3 compatible API).
...
| Component | Requirement                                                  |
| --------- | ------------------------------------------------------------ |
| CPU       | AMD EPYC 9004 'Genoa' Zen 4 or better with minimum 32 cores. |
