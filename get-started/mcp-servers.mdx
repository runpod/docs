...
- **[Runpod API MCP server](#runpod-api-mcp-server):** Manage Pods, endpoints, templates, volumes, and registries through the Runpod REST API. Authenticate with Sign in with Runpod or a [Runpod API key](/get-started/credentials#api-keys).
...
**Authentication:** Sign in with Runpod (OAuth) for the hosted server, or a [Runpod API key](/get-started/credentials#api-keys) for the local server or as a hosted-server override.
