...
<Note>
The skills plugin requires a coding agent with terminal access and Node.js installed (the install command uses `npx`). It can't be installed into chat apps like the ChatGPT or Claude mobile apps, which don't provide a shell.
</Note>
