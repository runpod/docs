...
- **Safety:** `VolumeCache` never follows or copies symbolic links. Before writing any archive member or large-file destination, it checks that the path resolves inside one of your configured `dirs`, so a mirror entry can't write outside your cached directories.
