...
You can tune the thresholds for these checks using environment variables, or disable them for testing. Your own registered checks always run regardless of these threshold settings.
...
| Network connectivity | All workers | Network connectivity via TCP to the Runpod API | 5 second timeout | `RUNPOD_NETWORK_CHECK_TIMEOUT` |
...
| GPU compute benchmark | GPU workers | Compute responsiveness (matrix multiply) | 2 seconds | `RUNPOD_GPU_BENCHMARK_TIMEOUT` |
...
Runpod runs the checks that don't need a running worker as early as possible. This way, an unhealthy worker fails before it spends time loading a large model. The memory, disk, and CUDA version checks and the native GPU memory allocation test run when your worker first imports `runpod`. If one of them fails, the worker exits and is marked unhealthy before your model loads.

The other checks need the running worker: network connectivity, CUDA device initialization, and the GPU compute benchmark. These run at worker start, along with your registered checks. Runpod checks disk space in both places, because loading a model between import and startup can use space that was free earlier.

This early pass runs only on the Runpod Serverless platform. It's skipped for realtime workers and during local development and testing.

The early checks read their thresholds when `runpod` is imported, so set any threshold variables in your Dockerfile or environment before the import. Values set later, from your handler code, apply only to the checks that run at worker start.

To run every check at worker start instead of splitting them, set `RUNPOD_DEFER_FITNESS_CHECKS=true`. This keeps all checks enabled and skips the early import-time pass.

<Note>
The early import-time pass and the `RUNPOD_DEFER_FITNESS_CHECKS` and `RUNPOD_SKIP_FITNESS_CHECKS` controls require a recent version of the `runpod` SDK. If your workers don't run checks at import time, upgrade the `runpod` SDK to a newer release.
</Note>
...
To disable fitness checks entirely, including your own registered checks, set `RUNPOD_SKIP_FITNESS_CHECKS=true`. No checks run at import time or at worker start. This is intended for testing only.
