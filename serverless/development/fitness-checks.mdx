...
<Note>
Fitness checks validate a worker's environment and mark the worker unhealthy so it exits. Use [prestart hooks](/serverless/development/prestart-hooks) instead to run startup work such as loading a model and surface its failures to the caller.
</Note>
