...
When the Runpod platform processes a job as part of a batch, the Runpod SDK's built-in logger prefixes each framework log message for that job with `[batchId=<id>]`, where `<id>` is the batch ID. For example, a message appears as `[batchId=abc123] <message>`.

This prefix is automatic, so there's no setting, parameter, or environment variable to configure. The Runpod SDK adds it only to framework log messages for batched jobs: standard jobs don't get the prefix, and the SDK never prefixes logs you write yourself with Python's `logging` library. Because the batch ID appears in the message text, you can scan the log stream for a batch ID to find all log entries that belong to the same batch.
