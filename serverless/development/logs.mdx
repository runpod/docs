...
Worker logs are temporary logs that exist only on the specific server where the worker is running. These logs are not throttled, but are not persistent, and are removed when a worker terminates. Unlike endpoint logs, worker logs are not centrally stored.
...
2. [SSH into a Pod](/pods/configuration/use-ssh) with the same network volume attached.
