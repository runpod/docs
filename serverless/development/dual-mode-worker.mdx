...
Once inside the Pod, navigate to `/app` and run your handler directly:

```sh
python handler.py
```

This will execute the Pod-specific test harness in your `handler.py`, giving you immediate feedback. You can edit `handler.py` within the Pod (using an attached volume or tools inside the Pod) and re-run it for rapid iteration.

## Step 8: Deploy to a Serverless endpoint
...
## Understanding the Pod-first development workflow

Congratulations! You've successfully built, deployed, and tested a dual-mode Serverless worker. Now, let's explore the recommended iteration process for this Pod-first development workflow:
