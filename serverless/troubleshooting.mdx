...
### Endpoint scaled down unexpectedly

Runpod scales endpoints down automatically in two situations:

- **Idle endpoints**: An endpoint that receives no requests for 3 days scales down to 2 workers, and an endpoint that receives no requests for 7 days scales down to 0 workers. See [Idle endpoint scale-down](/serverless/endpoints/endpoint-configurations#idle-endpoint-scale-down).
- **Persistent worker failures**: When a worker fails repeatedly, Runpod scales the endpoint down and sends you an email notification. Check your endpoint logs to diagnose the failures and fix the underlying issue.

In both cases, scale the endpoint back up manually when you're ready to use it again.
