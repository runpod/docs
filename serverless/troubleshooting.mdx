...
When the vLLM worker hits a fatal startup failure it recognizes, it keeps the worker alive instead of restarting silently, and returns a structured error on every job the endpoint sends it. Check the job response before digging through logs: the message names the cause and how to fix it.
...
- **The model wasn't found on Hugging Face.** Check `MODEL_NAME` for typos (it must be the full `org/repo` ID).
- **A pinned model revision doesn't exist.** Check `MODEL_REVISION` (and `TOKENIZER_REVISION`, if you set one) against the revisions listed on the model page, or unset it to use the default branch. If a revision that existed when you deployed later disappears (for example, when the repository's history is force-pushed), the worker automatically relaunches once against the current default branch, dropping the `MODEL_REVISION`, `TOKENIZER_REVISION`, and `CODE_REVISION` pins. It reports this error only if that second attempt also fails.
...
A GPU out-of-memory error is the most common recognized startup failure. It happens when the model weights or the KV cache don't fit in GPU memory, including the memory vLLM reserves up front for CUDA graphs.

When vLLM runs out of memory during startup, the worker relaunches it once with a smaller memory footprint before reporting an error:

- It sets `ENFORCE_EAGER=true`, which disables CUDA graphs and frees the memory reserved for them. This overrides an explicit `ENFORCE_EAGER=false`.
- It lowers `MAX_NUM_BATCHED_TOKENS` to `8192` when the variable is unset, set to `0`, or set higher than `8192`. Its default doubled to `16384` in vLLM v0.28, which doubles peak activation memory.

If the relaunch succeeds, the endpoint serves requests, but without CUDA graphs it can be slower than usual. The worker logs a warning for each setting it changed, so check the [logs](/serverless/development/logs) when an endpoint on a tight GPU is slower than expected. To avoid the extra launch, set `ENFORCE_EAGER=true` and `MAX_NUM_BATCHED_TOKENS=8192` yourself, or use one of the fixes below so the model fits with its default settings.

If `ENFORCE_EAGER` is already `true` and `MAX_NUM_BATCHED_TOKENS` is already `8192` or lower, the worker skips the relaunch because it would run out of memory the same way. It returns a `startup_error` right away (see [vLLM worker fails to start](#vllm-worker-fails-to-start)). If the relaunch also runs out of memory, the `startup_error` message states that the retry already ran. To make the model fit:
...
- If you set `KV_CACHE_DTYPE=fp8` on an Ampere or Ada GPU, switch it back to `auto`. On those GPUs, fp8 KV cache forces the FlashInfer backend, which needs extra workspace memory.
