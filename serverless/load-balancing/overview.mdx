...
All requests to load balancing endpoints require authentication with an [API key](/get-started/api-keys) in the Authorization header:

```bash
Authorization: Bearer YOUR_API_KEY
```

The required access level depends on the HTTP method:

- `POST` requests require an API key with ***Read/Write*** or ***All*** permissions.
- `GET` requests (such as health checks) can use ***Read Only*** permissions.
