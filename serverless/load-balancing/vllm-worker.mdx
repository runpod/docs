---
title: "Deploy a vLLM endpoint with load balancing"
sidebarTitle: "vLLM load balancer"
description: "Run the official vllm/vllm-openai image on a load balancing Serverless endpoint, or build a custom FastAPI vLLM server on Runpod."
---

This page covers two ways to run vLLM on a load balancing Serverless endpoint. The recommended, simplest path deploys the official `vllm/vllm-openai` image for an OpenAI-compatible server with no custom code. The advanced path builds a custom FastAPI server when you need a custom request and response contract.
...
## Deploy with the official vLLM image

Deploying the official `vllm/vllm-openai` image gives you a running OpenAI-compatible vLLM server with no Docker build and no custom code.
...
- A Runpod account.
- A Hugging Face access token, if you plan to serve a gated or private model.

<Steps>
  <Step title="Create the endpoint">
    On the [Serverless page](https://console.runpod.io/serverless), create a new endpoint and click **Import from Docker Registry**.
  </Step>
  <Step title="Set the container image">
    In the **Container Image** field, enter `vllm/vllm-openai:<VERSION>`. Pin a specific version tag rather than `latest` so your workers stay reproducible.
  </Step>
  <Step title="Set the endpoint type">
    Under **Endpoint Type**, select **Load Balancer**.
  </Step>
  <Step title="Set the vLLM arguments">
    In the container start command, provide the arguments passed to `vllm serve`. At minimum, set `--model` to the repo id of the model you want to serve, for example `--model meta-llama/Llama-3.1-8B-Instruct --max-model-len 8192`.
  </Step>
  <Step title="Set the environment variables">
    - Set `PORT` to `8000`. vLLM listens on port 8000, and the load balancer must route to that port.
    - Set `HF_TOKEN` to a Hugging Face access token if you're serving a gated or private model. Store it as a secret rather than a literal value. You can create a token in your Hugging Face account settings.
  </Step>
  <Step title="Expose the HTTP port">
    Under **Expose HTTP Ports (Max 10)**, add `8000/http`.
  </Step>
  <Step title="Confirm the health check">
    The official vLLM image already answers Runpod's default `/ping` health check (and also `/health`), so you don't need to configure anything for health checks. If you run an image whose server uses a different health path, override it with the **Health check endpoint** field or the `HEALTH_CHECK_PATH` environment variable. See [Health checks](/serverless/load-balancing/overview#health-checks) for how the load balancer uses this path.
  </Step>
  <Step title="Deploy the endpoint">
    Click **Deploy Endpoint** to create the endpoint and start your workers.
  </Step>
</Steps>

### Call your endpoint

Once a worker is healthy, you can call any vLLM route at `https://ENDPOINT_ID.api.runpod.ai/PATH`, authenticating with your Runpod API key. For example, to send a chat completion request:
...
The request `model` field must match the repo id you passed to `--model`, unless you set `--served-model-name`, in which case use that name.

Your workers take some time to initialize after you deploy. Track their progress in the **Workers** tab of your endpoint page. When a worker is healthy, the request returns a chat completion response.

<Tip>
Load balancing endpoints have no queue, so a request that arrives before any worker is ready returns an error rather than waiting. For production applications, implement a health check with retries before sending requests. See [Handling cold starts](/serverless/load-balancing/overview#handling-cold-starts) for a complete code example.
</Tip>

## Build a custom vLLM server

Build a custom vLLM server when you need a custom API contract or request preprocessing that the official image doesn't provide. The rest of this page walks through building a vLLM application with FastAPI and deploying it as a load balancing Serverless endpoint.
...
For production applications, implement a health check with retries before sending requests. See [Handling cold starts](/serverless/load-balancing/overview#handling-cold-starts) for a complete code example.
</Tip>
