title: "Build a vLLM load balancer"
sidebarTitle: "Build a vLLM load balancer"
description: "Learn how to build and deploy a vLLM load balancer."
tag: "BETA"
---
...
This tutorial shows how to build a load balancing vLLM worker using FastAPI and deploy it as a Serverless endpoint on Runpod.
...
This file defines the data contracts for your API. The `GenerationRequest` and `ChatCompletionRequest` models specify what data clients need to send, while `GenerationResponse` and `ErrorResponse` define what they'll receive back. Each model includes validation rules using Pydantic's `Field` function to ensure parameters stay within acceptable ranges.

## Step 3: Create utility functions

Next, we'll create helper functions that support the main application. These utilities handle common tasks like formatting chat prompts and creating standardized error responses.
...
Now we'll create the core FastAPI application that brings everything together. This file manages the vLLM engine, handles incoming requests, and serves your API endpoints.
...
This is the heart of your vLLM application. It creates a FastAPI server that manages the vLLM engine and exposes three API endpoints: 
...
Then add the following build steps to your `Dockerfile`:
...
## Step 6: Build and pushyour Docker image
...
## Step 6: Deploy a load balancing endpoint
...
Once your endpoint is created, you can access your vLLM APIs at:

```
https://ENDPOINT_ID.api.runpod.ai/PATH
```

For example, the load balancing worker we defined in step 4 exposes these endpoints:
...
You can now make requests to your load balancing vLLM endpoint. Replace `ENDPOINT_ID` and `RUNPOD_API_KEY` with your actual values:

To run a health check, use this command:
...
This means your workers did not initialize in time to process the request. Try running the request again.
</Tip>

Congrats! You've created a load balancing vLLM endpoint and used it to serve a large language model.

<Note>
You can find a Python script for to comprehensive local testing of your vLLM load balancer [on GitHub](https://github.com/runpod-workers/vllm-loadbalancer-ep/blob/main/example.py).
</Note>

## Next steps

Now that you've deployed a vLLM load balancer, you can try:
