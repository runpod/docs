...
description: "Deploy a load balancing Serverless endpoint on Runpod."
tag: "BETA"
---

# Build a load balancing Serverless worker

This tutorial will guide you through creating a load balancing Serverless endpoint on Runpod. Unlike traditional queue-based endpoints, load balancing endpoints allow you to expose custom REST APIs directly, giving you complete control over your API's structure and behavior, while still leveraging the scalability and reliability of Runpod's Serverless infrastructure.

## What you'll learn

In this tutorial you'll learn how to:

- Understand the difference between queue-based and load balancing endpoints.
...
```bash
# Build the image
docker build -t YOUR_DOCKER_USERNAME/loadbalancer-example:v1.0 .
...
4. In the **Container Image** field, enter your Docker image URL: `yourusername/loadbalancer-example:v1.0` 
5. Give your endpoint a name.
6. Under **Endpoint Type**, selecent **Load Balancer**.
7. Under **Worker Configuration**, select at least one GPU type.
...
To make a request to the mock generation endpoint, run this command, replacing `ENDPOINT_ID` with your actual endpoint ID:
...
2. Set environment variables (**Public Environment Variables** in the endpoint configuration form) to:
    - `PORT_HEALTH`: `5000`
    - `PORT`: `8000`
...
- **API not accessible**: Verify that your endpoint type is set to "Load Balancer".
- **Port issues**: Make sure the environment variables `PORT_HEALTH` and `PORT` match what your application is using.
