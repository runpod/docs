...
{/* DOES THIS BY DEFAULT: N. Expand **Container Configuration** and under **Expose HTTP Ports (Max 10)**, add `5000` to the list. */}
8. Expand **Environment Variables** and add the key `PORT` with the value `5000` to the list.
...
```bash ping
curl -X POST "https://ENDPOINT_ID.api.runpod.ai/ping" \
    -H 'Authorization: Bearer RUNPOD_API_KEY' \
    -H "Content-Type: application/json" \
```

```bash stats
curl -X POST "https://ENDPOINT_ID.api.runpod.ai/stats" \
    -H 'Authorization: Bearer RUNPOD_API_KEY' \
    -H "Content-Type: application/json" \
```
...
If you send this request, your workers will take some time to initialize (you can check the logs in the **Workers** tab of your endpoint page to track their progress) before returning the following response:

```json
{"generated_text":"Response to: Hello, world! (request #1)"}%   
```
