...
| `MAX_MODEL_LEN` | `auto` | `int`, `auto` | Maximum context length in tokens the engine allocates KV cache for. Set an integer for a fixed value, or use `auto` to let the engine pick the largest length that fits in available VRAM, based on the model's configuration. Human-readable size suffixes such as `16k` (1000-based) and `16K` (1024-based) are also accepted. Lower it to reduce VRAM usage; raise it for long-context models that support it, if you have the VRAM. |
