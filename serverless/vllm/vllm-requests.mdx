...
When deployed to a queue-based endpoint, Runpod's official vLLM worker uses the same `/run` and `/runsync` operations as other Runpod endpoints, following the standard [Serverless request structure](/serverless/endpoints/send-requests).
...
vLLM endpoints support both synchronous and asynchronous requests using the `/run` and `/runsync` operations.
