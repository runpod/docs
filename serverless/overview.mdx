...
import { JobTooltip, LoadBalancingEndpointTooltip, QueueBasedEndpointsTooltip, InferenceTooltip } from "/snippets/tooltips.jsx";
...
Prestart hooks run startup work such as loading a model or booting an inference engine before your handler serves jobs, and the SDK supervises them. If a hook fails or times out, the SDK reports the failure to the caller instead of leaving requests stuck in the `IN_QUEUE` state with the reason buried in the worker logs.

[Learn more about prestart hooks](/serverless/development/prestart-hooks)
