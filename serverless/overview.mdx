...
3. [Use network volumes](/serverless/storage/network-volumes): Network volumes provide persistent storage, but models loaded from network storage are slower than cached or baked models. Use this option only when the above approaches don't fit your needs.
