...
| **Data center availability** | Available in any data center | Tied to a single data center |
...
- You want the same model available across multiple data centers, rather than tied to a single one.
...
If your image references the baked-in model through a hardcoded path, update that reference to point to the Model Repo path instead. How much you need to change depends on how your image currently loads the model.
