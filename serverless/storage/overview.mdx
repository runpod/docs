...
All data saved by a worker's <HandlerFunctionTooltip /> is stored in the container disk by default. To persist data beyond the current worker session, use a network volume or S3-compatible storage.
