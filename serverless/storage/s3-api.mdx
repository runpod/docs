...
```bash
aws s3 ls --endpoint-url https://s3api-[DATACENTER].runpod.io/ \ 
...
```bash
aws s3 cp --endpoint-url https://s3api-[DATACENTER].runpod.io/ \
    local-file.txt \
...
```bash
aws s3 rm --endpoint-url https://s3api-[DATACENTER].runpod.io/ \
...
```bash
aws s3 sync --endpoint-url https://s3api-[DATACENTER].runpod.io/ \
    ./[LOCAL_DIR] \
    s3://[NETWORK_VOLUME_ID]/[REMOTE_DIR]
```
