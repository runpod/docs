...
**Billing:** Using the S3-compatible API does not affect pricing. Network volumes are billed hourly at $0.07/GB/month for the first 1TB, and $0.05/GB/month for additional storage.
...
`ls` operations may timeout when used on network volumes containing many files (over 50,000 files) or large amounts of data. For more information, see [S3-compatible API limitations](#listobjects-timeout-with-large-file-counts).
...
### ListObjects timeout with large file counts

The `ListObjects` and `ListObjectsV2` operations may timeout when used on network volumes containing many files (over 50,000 files) or large amounts of data.

**Symptoms:**
- `aws s3 ls` commands result in `504 Gateway Timeout` errors.
- File uploads and downloads work normally, but you cannot list directory contents.
- The issue primarily affects network volumes that were populated using Pod templates or other non-S3 methods.

**Workarounds:**
1. **Use alternative access methods**: Access your network volume [through a Pod](/pods/storage/create-network-volumes) instead of the S3 API when browsing large directories.
2. **Target specific subdirectories**: If possible, list specific subdirectories rather than the root directory to reduce the number of files being processed.
