---
title: "Manage network volumes with the S3-compatible API"
...
<Steps>
  <Step title="Create a shared key">
    ...
    4. Make a note of your access key and secret access key for the next step.
  ...
  <Step title="Configure AWS CLI">
    ...
    3.  Provide the following when prompted:
        *   **AWS Access Key ID**: Enter your RunPod Shared Key ID.
        *   **AWS Secret Access Key**: Enter your RunPod Shared Secret Access Key.
        *   **Default region name**: You can leave this blank.
        *   **Default output format**: You can leave this blank or set it to `json`.

    This command configures the AWS CLI to use your RunPod shared key by storing these details in your AWS credentials file (typically at `~/.aws/credentials`).
...
You can interact with your RunPod network volumes using standard S3 tools:

*   [Standard AWS CLI commands](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3/index.html).
*   [The Boto3 Python library](https://boto3.amazonaws.com/v1/documentation/api/latest/index.html).

Core operations such as `ls`, `cp`, `mv`, `rm`, and `sync` function as expected.

### AWS CLI examples

When using AWS CLI commands, you must pass your RunPod endpoint URL using the `--endpoint-url` parameter. Your endpoint ID can be found in the RunPod console.

#### List objects

Use `ls` to list objects in a network volume:

```bash
aws s3 ls --endpoint-url https://api.runpod.ai/v2/[ENDPOINT_ID] s3://network-volume-id/path
```
...
```bash
aws s3 cp --endpoint-url https://api.runpod.ai/v2/[ENDPOINT_ID] localfile.txt s3://network-volume-id/path
```

Use `cp` to copy a file from a network volume to a local directory:

```bash
aws s3 cp --endpoint-url https://api.runpod.ai/v2/[ENDPOINT_ID] s3://network-volume-id/path/file.txt ./
```

Use `rm` to remove a file from a network volume:

```bash
aws s3 rm --endpoint-url https://api.runpod.ai/v2/[ENDPOINT_ID] s3://network-volume-id/path/file.txt
```

#### Sync directories

This command syncs a local directory (source) to a network volume (destination):

```bash
aws s3 sync ./local-dir s3://network-volume-id/path
```

This command syncs a network volume (source) to an endpoint (destination):

```bash
aws s3 sync s3://network-volume-id/path --endpoint-url https://api.runpod.ai/v2/[ENDPOINT_ID]
```
