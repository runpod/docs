...
The script takes command-line arguments for the network volume ID (as an S3 bucket), the datacenter-specific S3 endpoint URL, the local file path, the desired object key (file path on the volume), and the AWS region (which corresponds to the RunPod datacenter ID). It requires your RunPod shared key credentials to be set as environment variables:
...
def main():
    ...
    parser.add_argument(
        "-e", "--endpoint",
        required=True,
        help="The S3 endpoint URL for your RunPod datacenter (e.g., 'https://<DATACENTER_ID>-s3api.runpod.dev/')."
    )
    parser.add_argument(
        "-f", "--file",
        required=True,
