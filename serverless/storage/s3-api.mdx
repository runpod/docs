...
| `ListObjects`            | List objects in a bucket. Unlike standard S3, this includes empty directories.                    |
...
When running `aws s3 ls` or `ListObjects` on a large directory for the first time, it may run very slowly, or you may encounter the following error: 

```bash
"fatal error: Error during pagination: The same next token was received twice: ..."
```

This occurs because Runpod must compute and cache the MD5 checksum (i.e., ETag) for files created without the S3-compatible API. For directories containing 10,000+ files or 10+ GB of data, this computation can take several minutes, and any `ListObjects` or `aws s3 ls` request that accesses such files must wait until the checksum is ready.
