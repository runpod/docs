...
| `GetObject`              | Download objects (<=512MB).                   |
...
| `PutObject`              | Upload objects (<=512MB).                       |
...
Files larger than 512MB must be transferred using multipart uploads.
...
-   The maximum size for a single part of a multipart upload is 512MB.
