...
If you encounter issues with the S3-compatible API, try the following:

-   **Network volume size**: Ensure your network volume has enough free space for the operation you're trying to perform.
-   **Part size**: If your multipart uploads are failing, ensure that you're using a part size of 500MB or less.
-   **Bad gateway error**: If you enounter a 502 "bad gateway" error during a file transfer, try increasing `AWS_MAX_ATTEMPTS` to 10 or more:
    ```bash
