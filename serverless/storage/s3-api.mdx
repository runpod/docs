...
<Steps>
  ...
  <Step title="Configure AWS CLI">
    ...
    3.  Provide the following when prompted:
        *   **AWS Access Key ID**: Enter your RunPod shared key's access key ID.
        *   **AWS Secret Access Key**: Enter your RunPod shared key's secret access key.
...
The S3-compatible API is currently available for network volumes hosted in a limited number of datacenters. Each datacenter has an endpoint URL that you'll use when calling the S3-compatible API, which uses the format `https://s3api-[DATACENTER].runpod.dev/`.

| Datacenter | Endpoint URL |
|-------------|--------------|
| `EUR-IS-1` | `https://s3api-eur-is-1.runpod.dev/` |
| `EU-RO-1` | `https://s3api-eu-ro-1.runpod.dev/` |


## AWS CLI examples

When using AWS CLI commands, you must pass in the URL for the RunPod datacenter where your network volume is located using the `--endpoint-url` parameter ([listed above](#datacenter-availability)).

You can find the datacenter ID for your network volume in the [Storage section](https://www.runpod.io/console/user/storage) of the RunPod console.
...
```bash
aws s3 ls --endpoint-url https://s3api-[DATACENTER].runpod.dev/ \ 
...
```bash
aws s3 cp --endpoint-url https://s3api-[DATACENTER].runpod.dev/ \
    local-file.txt \
...
```bash
aws s3 rm --endpoint-url https://s3api-[DATACENTER].runpod.dev/ \
...
```bash
aws s3 sync --endpoint-url https://s3api-[DATACENTER].runpod.dev/ \
    ./[LOCAL_DIR] \
    s3://[NETWORK_VOLUME_ID]/[REMOTE_DIR]
```
...
Your RunPod shared key credentials must be set as environment variables:

*   `RUNPOD_USER_EMAIL`: Should be set to your RunPod account email.
*   `RUNPOD_S3_API_KEY`: Should be set to your RunPod shared key's secret access key.
...
def create_s3_client(region: str, endpoint_url: str):
    ...
    #                       (e.g., 'https://ca-qc-1-s3api.runpod.dev/').

    # Returns:
    #   boto3.client: An S3 client object, configured for the RunPod S3 API.

    # Retrieve RunPod shared key credentials from environment variables.
    aws_access_key_id = os.environ.get("RUNPOD_USER_EMAIL")
    aws_secret_access_key = os.environ.get("RUNPOD_S3_API_KEY")

    # Ensure necessary shared key credentials are set in the environment
    if not aws_access_key_id or not aws_secret_access_key:
        raise EnvironmentError(
            "Please set RUNPOD_USER_EMAIL (with Shared Key Access Key) and "
            "RUNPOD_S3_API_KEY (with Shared Key Secret Access Key) environment variables. "
            "These are obtained from 'Shared Keys' in the RunPod console settings."
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload a file to a RunPod Network Volume using its S3-compatible API. "
                    ...
                    "with your RunPod Shared Key credentials."
    )
    parser.add_argument(
        "-b", "--bucket",
        required=True,
    ...
    parser.add_argument(
        "-e", "--endpoint",
        required=True,
        help="The S3 endpoint URL for your RunPod datacenter (e.g., 'https://s3api-[DATACENTER].runpod.dev/')."
    )
    parser.add_argument(
        "-f", "--file",
        required=True,
...
```bash
./s3_example_put.py --endpoint https://s3api-eur-is-1.runpod.dev/ \
    --region 'EUR-IS-1' \
