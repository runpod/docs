...
<Warning>
the DataCenter `--region` must be capitalised and passed, so the command can be passed as `aws s3 ls --endpoint-url https://s3api-eu-ro-1.runpod.io --region EU-RO-1` <br><br>

&#x2612; flags like `--region eu-ro-1` are incorrect and will not work <br>
&#9745; the region flags must be `--region EU-RO-1` or ÈUR-IS-1` <br>
...
| `PutObject`              | Upload objects (<500MB).                       |
...
Files larger than 500MB must be uploaded using multipart uploads.
