...
<Step title="Create an S3 API key">
  ...
  <Step title="Configure AWS CLI">
    ...
    <Tip>
    **Verify your configuration**: After configuring your credentials, you can verify what credentials are currently being used by running:
    
    ```bash
    aws configure list
    ```
    
    This command shows which credentials are active and their source (config file, environment variables, etc.). Note that environment variables like `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` will override your config file settings if they are set.
    </Tip>
    
  </Step>
</Steps>

## Understanding network volume path mapping

Network volumes are mounted to Serverless workers at `/runpod-volume`, and to Pods at `/workspace` by default. The S3-compatible API maps these paths as follows:
...
- Which credentials are currently active
- The source of each credential (config file, environment variables, etc.)
- Whether all required credentials are properly set
