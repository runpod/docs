...
The S3-compatible API is currently available for network volumes hosted in these datacenters:
...
Each datacenter has an endpoint URL that is required when using the S3-compatible API. The URL uses the format `https://s3api-[DATACENTER].runpod.dev/`, for example `https://s3api-eur-is-1.runpod.dev/`.
