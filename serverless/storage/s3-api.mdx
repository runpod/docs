...
The following table shows the current implementation status of S3 API operations and AWS CLI commands. Use the tables below to understand what functionality is available and plan your development workflows accordingly.

<Note>
If a function is not listed in this matrix, it means it's not currently implemented. We continuously expand S3 API support based on customer needs and usage patterns.
</Note>
| ------------------------- | -------------------------------------------- |
### Core operations

| S3 API Operation | Support | AWS CLI | Notes |
|------------------|--------|-----------------|-------|
| `CopyObject` | ✅ Implemented | `aws s3 cp`, `aws s3 sync` | Copy objects between locations |
| `DeleteObject` | ✅ Implemented | `aws s3 rm` | Remove individual objects |
| `GetObject` | ✅ Implemented | `aws s3 cp`, `aws s3api get-object` | Download objects |
| `HeadBucket` | ✅ Implemented | `aws s3 ls` | Verify bucket exists and permissions |
| `HeadObject` | ✅ Implemented | `aws s3api head-object` | Retrieve object metadata |
| `ListBuckets` | ✅ Implemented | `aws s3 ls` | List available network volumes |
| `ListObjects` | ✅ Implemented | `aws s3 ls` | List objects in a bucket (includes empty directories) |
| `ListObjectsV2` | ✅ Implemented | `aws s3 ls` | Enhanced version of ListObjects |
| `PutObject` | ✅ Implemented | `aws s3 cp`, `aws s3api put-object` | Upload objects (\<500MB) |
...
| S3 API Operation | Status | AWS CLI | Notes |
|------------------|--------|-----------------|-------|
| `CreateMultipartUpload` | ✅ Implemented | `aws s3 cp` (automatic) | Start multipart upload for large files |
| `UploadPart` | ✅ Implemented | `aws s3 cp` (automatic) | Upload individual parts |
| `CompleteMultipartUpload` | ✅ Implemented | `aws s3 cp` (automatic) | Finish multipart upload |
| `AbortMultipartUpload` | ✅ Implemented | `aws s3api abort-multipart-upload` | Cancel multipart upload |
| `ListMultipartUploads` | ✅ Implemented | `aws s3api list-multipart-uploads` | View in-progress uploads |
| `ListParts` | ✅ Implemented | `aws s3api list-parts` | List parts of a multipart upload |

### Batch and advanced operations

| S3 API Operation | Status | AWS CLI | Notes |
|------------------|--------|-----------------|-------|
| `DeleteObjects` | ❌ Not implemented | `aws s3 rm --recursive` | Batch delete multiple objects |
| `RestoreObject` | ❌ Not implemented | N/A | Restore archived objects |

### Bucket management operations

| S3 API Operation | Status | AWS CLI Support | Notes |
|------------------|--------|-----------------|-------|
| `CreateBucket` | ❌ Not implemented | N/A | Use RunPod console to create network volumes |
| `DeleteBucket` | ❌ Not implemented | N/A | Use RunPod console to delete network volumes |
| `GetBucketLocation` | ❌ Not implemented | N/A | Datacenter info available in console |
| `GetBucketVersioning` | ❌ Not implemented | N/A | Versioning not supported |
| `PutBucketVersioning` | ❌ Not implemented | N/A | Versioning not supported |

### Access control and permissions

| S3 API Operation | Status | AWS CLI Support | Notes |
|------------------|--------|-----------------|-------|
| `GetBucketAcl` | ❌ Not implemented | N/A | ACLs not supported |
| `PutBucketAcl` | ❌ Not implemented | N/A | ACLs not supported |
| `GetObjectAcl` | ❌ Not implemented | N/A | ACLs not supported |
| `PutObjectAcl` | ❌ Not implemented | N/A | ACLs not supported |
| `GetBucketPolicy` | ❌ Not implemented | N/A | Bucket policies not supported |
| `PutBucketPolicy` | ❌ Not implemented | N/A | Bucket policies not supported |

### Object metadata and tagging

| S3 API Operation | Status | AWS CLI Support | Notes |
|------------------|--------|-----------------|-------|
| `GetObjectTagging` | ❌ Not implemented | N/A | Object tagging not supported |
| `PutObjectTagging` | ❌ Not implemented | N/A | Object tagging not supported |
| `DeleteObjectTagging` | ❌ Not implemented | N/A | Object tagging not supported |

### Encryption and security

| S3 API Operation | Status | AWS CLI Support | Notes |
|------------------|--------|-----------------|-------|
| `GetBucketEncryption` | ❌ Not implemented | N/A | Encryption not supported |
| `PutBucketEncryption` | ❌ Not implemented | N/A | Encryption not supported |
| `GetObjectLockConfiguration` | ❌ Not implemented | N/A | Object locking not supported |
| `PutObjectLockConfiguration` | ❌ Not implemented | N/A | Object locking not supported |
...
-   Object locking.
-   Website redirects.
-   Storage classes besides `STANDARD`.
