...
*   **Object names**: Unlike traditional S3 key-value stores, RunPod object names in correspond to actual file paths on your network volume.
*   **Multipart uploads**:
...
*   **Storage capacity**: Network volumes have a fixed storage capacity, unlike the virtually unlimited storage of standard S3 buckets.
    *   Upload and copy actions do not check for available free space beforehand and may fail if the volume runs out of space. This behavior is similar to applying a size quota in S3.
...
*   **Special characters in object names**: Object names containing special characters (e.g., `#`) may need to be URL encoded to ensure proper processing.
