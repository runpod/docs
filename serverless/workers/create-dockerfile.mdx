...
<Tip>
For GPU workers using the Runpod SDK's fitness checks, install the GPU test binary after installing the package: `pip install runpod && runpod install-gpu-test`. See [Fitness checks](/serverless/workers/fitness-checks#installing-the-gpu-test-binary) for details.
</Tip>
