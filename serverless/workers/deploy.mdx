...
## Deploying with AWS ECR

Runpod does not officially support deploying images from AWS ECR (Elastic Container Registry). When you authenticate with ECR, AWS provides a temporary token that's valid for only 12 hours. After expiration, Runpod won't be able to pull your container image, causing deployment failures. However, this is possible to implement by running an AWS Lambda function that:

- Generates fresh ECR authentication tokens regularly.
- Updates the container registry credentials on Runpod's side before they expire.
- Runs frequently enough to ensure credentials never expire (every few hours).

{/* TODO: then you can deploy your worker image directly from your ECR repository using the serverless UI (container image: link to ECR repo, registry credentials: AWS credentials). */}
