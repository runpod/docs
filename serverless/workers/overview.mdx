...
| **Unhealthy** | Crashed; auto-retries for up to 7 days. After persistent failures, the endpoint scales down and Runpod sends you an email notification. | No |

View worker states in the **Workers** tab of your endpoint in the [Runpod console](https://www.console.runpod.io/serverless).

<Note>
Runpod automatically scales down idle endpoints: to 2 workers after 3 days with no requests, and to 0 workers after 7 days with no requests. To use the endpoint again, scale it back up manually. For details, see [Idle endpoint scale-down](/serverless/endpoints/endpoint-configurations#idle-endpoint-scale-down).
</Note>
