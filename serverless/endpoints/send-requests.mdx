...
If no worker is immediately available, the job is queued. Once a worker is available, the job is processed by the worker using your [handler function](/serverless/workers/handler-functions).
...
<Note>
This is only available for `cURL` and standard HTTP request APIs/libraries.
</Note>
...
`/run` requests return a response with the job ID and status:
...
Further results must be retrieved using the `/status` endpoint.
