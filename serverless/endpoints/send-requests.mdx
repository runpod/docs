...
description: "Learn how to send API requests to submit and manage jobs for your queue-based endpoints"
---
...
Serverless endpoints provide synchronous and asynchronous job processing with automatic worker scaling based on demand. This page covers everything from basic request structure to advanced operations, monitoring, and troubleshooting for queue-based endpoints.

## Key concepts

After you create a Serverless endpoint, you can **send requests** to the endpoint to **submit jobs and retrieve results**. The request can include parameters, payloads, and headers that define what the endpoint should process. For example, a `POST` request to submit a job, or a `GET` request to check status of a job, retrieve results, or check endpoint health.

A **job** is a unit of work that's ready for processing by your [workers](/serverless/workers/overview), containing the input data from the request. If no worker is immediately available, the job is queued. Once a worker is available, the job is processed by a worker using your [handler function](/serverless/workers/handler-functions).
...
Serverless endpoints provide a fixed set of operations for submitting and managing jobs. You can find a full list of operations in the [sections below](/serverless/endpoints/send-requests#operation-overview).
...
<Tip>
You can also send requests using standard APIs and libraries, like `fetch` (for JavaScript) and `requests` (for Python).
</Tip>
