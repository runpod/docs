...
You can select from a list of cached AI/ML models using the **Model (optional)** field. Selecting a model signals the system to place your workers on host machines that contain the selected model, resulting in faster cold starts and cost savings (since you won't be charged while the model is downloading). See [Pre-cached models](/storage/model-caching) for more details.
