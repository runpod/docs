...
Idle timeout applies to a single worker after it finishes a request. Separately, Runpod scales down an entire endpoint that goes for days without receiving any requests:

- After 3 days with no requests, the endpoint scales down to 2 workers.
- After 7 days with no requests, the endpoint scales down to 0 workers.

This scale-down is automatic and system-driven. To use an idle endpoint again, scale it back up manually.
