...
The TTL controls how long a job's data is retained in the system, covering **both queue time and execution time**. The default is 24 hours. The minimum is 10 seconds and maximum is 7 days.
...
You can override this on a per-request basis using the `ttl` field in the [job policy](/serverless/endpoints/send-requests#execution-policies), or configure it in the **Advanced** section of your endpoint settings.
