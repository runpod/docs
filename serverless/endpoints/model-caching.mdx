...
## Access cached models in your application

Cached models are stored at `/runpod-volume/huggingface-cache/hub/` following Hugging Face cache conventions. The directory structure replaces forward slashes (`/`) from the original model name with double dashes (`--`), and includes a version hash subdirectory.
...
## Programmatically find cached models

To dynamically locate cached models without hardcoding paths, use this helper function to scan the cache directory:

```python find_model_path.py
import os
...
def find_model_path(model_name):
    ...
    Args:
        model_name: The model name from Hugging Face (e.g., 'Qwen/Qwen2.5-0.5B-Instruct')
...
This function is useful when you're working with multiple models, need to verify that a model is available before loading it, or are building tools that discover available cached models.
