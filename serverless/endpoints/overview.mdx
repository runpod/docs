...
Endpoints are RESTful APIs that accept [HTTP requests](/serverless/endpoints/send-requests), execute your code using your [handler function](/serverless/workers/handler-functions), and return the result via HTTP response. Each endpoint provides a unique URL and abstracts away the complexity of managing individual GPUs/CPUs.

## Key features

### Execution modes (queue-based endpoints)

Queue-based endpoints offer **asynchronous processing** via the `/run` endpoint operation, which lets you submit jobs that run in the background and check results later, making this ideal for long-running tasks.
...
To learn more, see the [Operation overview](/serverless/endpoints/send-requests#operation-overview).

### Load balancing endpoints

Alternatively, load balancing endpoints offer **direct HTTP access** to your worker's HTTP server, bypassing the queueing system. This is ideal for real-time applications and streaming, but provides no queuing mechanism for request backlog, similar to UDP's behavior in networking. You can define your own custom API endpoints without a handler function, using any HTTP framework of your choice (like FastAPI or Flask).
...
To deploy an endpoint, make sure you have a working [handler function](/serverless/workers/handler-functions) and [Dockerfile ready](/serverless/workers/create-dockerfile).
