...
Runpod CLI prints machine-readable output, so it's easy to script or drive with an automated agent. Commands return JSON by default. Use the global `-o` (or `--output`) flag to switch to YAML:

```bash
runpodctl serverless list -o yaml
```

The only accepted values are `json` and `yaml`. Matching is case-insensitive and surrounding whitespace is trimmed, so `YAML` or `Yaml` works too. Any other value (such as `table` or `yml`) is rejected as a usage error (see [Error format](#error-format)).

### Error format

When a command fails, `runpodctl` writes a single flat JSON error object to stderr and exits with a non-zero status. The object always includes an `error` message, a stable `code` you can branch on, and an HTTP `status` when the failure came from an API call:

```json
{"error":"failed to get endpoint: endpoint not found","code":"not_found","status":404}
```

The `code` field stays stable across releases, so you can handle failures programmatically without parsing the error message. Common codes include `not_found`, `unauthorized`, `forbidden`, `bad_request`, `conflict`, `rate_limited`, `server_error`, and `usage_error`.

Usage errors—such as a missing argument, an unknown flag, or an unknown command—are tagged with `"code":"usage_error"` and are followed by the command's usage text on stderr:
