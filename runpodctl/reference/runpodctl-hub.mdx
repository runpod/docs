...
The CLI automatically applies the deployment constraints defined in the Hub release, including the compute type, GPU count, minimum CUDA version, GPU IDs, container disk size, and environment variables. Any flag you set explicitly takes precedence over the corresponding Hub value, so use flags like `--gpu-id`, `--compute-type`, or `--gpu-count` to override an individual constraint. See [`runpodctl serverless create`](/runpodctl/reference/runpodctl-serverless) for the full list of flags.
