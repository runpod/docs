...
### View endpoint environment overrides

Show the environment-variable override configured directly on the endpoint: the values that reach the endpoint's workers.

```bash
runpodctl serverless env <endpoint-id>
```

This is the only command that exposes the endpoint's own override. `serverless get` and `serverless list` do not include it, and `serverless get --include-template` shows only the template's default environment variables, which are a different set of values than the override that `serverless env` returns.

Every `runpodctl serverless` subcommand accepts the global `--output, -o` flag (`json` or `yaml`), which defaults to `json`.
...
An empty `envOverride` array (`[]`) is normal, not an error. It means the endpoint uses its template's default environment variables.

</Note>

<Warning>

The override can contain secrets such as API keys or tokens (for example an `HF_TOKEN`), so do not paste the command's output verbatim into tickets or chat.
