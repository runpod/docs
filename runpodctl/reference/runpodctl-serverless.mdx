...
Wait for the endpoint to have a ready worker before returning, instead of returning as soon as the endpoint is created. Readiness is reported when the endpoint's `/health` shows a ready or running worker; a ready worker may be one started via `--flash-boot` from a cached image. Setting `--workers-min 1` is the fastest way to get a ready worker. On timeout or interrupt (`Ctrl+C`), the endpoint is not deleted; only the wait stops.
</ResponseField>

<ResponseField name="--wait-timeout" type="string" default="10m">
Maximum time to wait when `--wait` is set (e.g., `90s`, `10m`, `1h`, `7d`). If the timeout is reached, the endpoint is not deleted and only the wait stops.
</ResponseField>
