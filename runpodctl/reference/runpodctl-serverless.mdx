...
When you deploy with `--hub-id`, the CLI automatically applies the deployment constraints defined in the Hub release: the compute type (`runsOn`), the number of GPUs per worker, the minimum CUDA version, GPU IDs, container disk size, and environment variables. Any flag you set explicitly overrides the matching Hub value, so use `--compute-type`, `--gpu-count`, `--min-cuda-version`, `--gpu-id`, or `--env` to change an individual constraint. If a Hub release lists more than one allowed CUDA version, the first one becomes the endpoint's minimum CUDA requirement. Endpoint creation fails if the release has no built image (it may still be building), specifies an unsupported compute type, or has a malformed deployment configuration. If an explicit flag conflicts with a Hub-supplied compute type, the error names the Hub listing as the source of the constraint.
