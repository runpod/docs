...
You must provide one of `--input` or `--input-file`, and they're mutually exclusive. The payload must be a JSON object. `runpodctl` rejects arrays, scalars, and `null` locally as a usage error before it sends any request.
...
Submit the job and print its ID without waiting. Equivalent to `--wait 0`.
</ResponseField>
