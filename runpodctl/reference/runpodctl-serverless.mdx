...
### Invoke URLs in command output

The `create`, `get`, `list`, and `update` commands include a `urls` object in their output with the ready-to-call `run`, `runsync`, and `health` URLs for each endpoint. This lets you call an endpoint right after creating it, without looking up its URLs in the web interface:
...
The URLs are derived from the endpoint ID, so they appear in the output even before the endpoint finishes provisioning. To poll a specific job, use the job status URL pattern shown above.
