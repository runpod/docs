...
You can also set model references (cached models) for an endpoint. Model references tell Runpod which models to cache on the hosts running your workers, which reduces cold start times. Setting model references with `--model-reference` replaces any existing model references on the endpoint. For details on how model caching works, see [Cached models](/serverless/endpoints/model-caching).

```bash
# Set a single model reference (replaces any existing references)
runpodctl serverless update <endpoint-id> --model-reference https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct:main

# Set multiple model references
runpodctl serverless update <endpoint-id> \
  --model-reference https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct:main \
  --model-reference https://huggingface.co/meta-llama/Llama-2-7b-chat-hf:main

# Remove all model references
...
Model reference (cached model) to attach to the endpoint. Repeat the flag to attach multiple models. This replaces any existing model references on the endpoint. Cannot be combined with `--clear-models`.
</ResponseField>
