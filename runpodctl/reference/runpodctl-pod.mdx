...
Wait for the Pod to become reachable before returning, instead of returning as soon as it is scheduled. Reachability is confirmed when the Pod's public port 22 accepts a TCP connection and returns an SSH banner — a connectivity check, not a key-authenticated SSH login, so it can succeed even if your public key was not added to the Pod. Requires a public port 22 (add `--public-ip` on Community Cloud) and is incompatible with `--ssh=false`. On timeout or interrupt (`Ctrl+C`), the Pod is left running and only the wait stops.
</ResponseField>

<ResponseField name="--wait-timeout" type="string" default="10m">
Maximum time to wait when `--wait` is set (e.g., `90s`, `10m`, `1h`, `7d`). If the timeout is reached, the Pod is left running and only the wait stops.
</ResponseField>
