...
Two flags add more detail to the output. `--include-network-volume` adds a `networkVolume` object describing the attached network volume, and `--include-machine` adds machine information. Whenever a Pod has a network volume attached, the output also includes a `networkVolumeId` field, whether or not you pass `--include-network-volume`. If the Pod has no network volume, the output omits both `networkVolumeId` and the `networkVolume` object.

#### Get flags

<ResponseField name="--include-network-volume" type="bool">
Include network volume details in the output. It adds a `networkVolume` object describing the attached network volume.
</ResponseField>

<ResponseField name="--include-machine" type="bool">
Include machine information in the output.
</ResponseField>

Example `networkVolume` object:
