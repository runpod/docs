...
When you create a Pod from a template with `--template-id`, the Pod inherits the template's container disk size, volume size, and environment variables unless you override them. To set a different size for this Pod, pass `--container-disk-in-gb` or `--volume-in-gb`; if you omit either flag, the template's value is used.

When you pass `--env`, your variables are merged with the template's. Your values take precedence for any keys the template also defines, and the template's other variables are preserved.
...
Container disk size in GB. When you create a Pod from a template and omit this flag, the Pod inherits the template's container disk size instead of the default.
</ResponseField>

<ResponseField name="--volume-in-gb" type="int">
Persistent volume size in GB. When you create a Pod from a template and omit this flag, the Pod inherits the template's volume size.
</ResponseField>
...
Environment variables as a JSON object (e.g., `'{"KEY":"value"}'`). When you create a Pod from a template, these variables are merged with the template's, and your values override the template's for any matching keys.
</ResponseField>
