...
<ResponseField name="--env" type="string">
Environment variables to update as a JSON object (e.g., `'{"KEY":"value"}'`). Values are merged into the Pod's existing environment: specified keys are updated while existing keys (including system keys like `PUBLIC_KEY`) are preserved.
</ResponseField>
