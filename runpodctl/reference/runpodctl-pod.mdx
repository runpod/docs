...
Output is JSON Lines: one JSON object per line, shaped `{source, line, ts}`, so it pipes cleanly into `jq` or is read by an agent. The `source` field is either `container` (your workload's stdout/stderr) or `system` (platform lifecycle events such as image pull and container create, start, and stop).

Logs outlive the workload: a stopped Pod still returns its full history, including the container-stop lines, so you can debug a Pod after it has stopped.

```bash
# Read a Pod's recent logs
runpodctl pod logs abc123

# Stream only new lines as they arrive (no history)
...
```json
{"source":"system","line":"Container started","ts":"2026-08-18T12:00:00Z"}
{"source":"container","line":"Model loaded","ts":"2026-08-18T12:00:01Z"}
```

Only the JSON Lines log records go to stdout; reconnection notices and errors go to stderr, so stdout stays clean for piping.

#### Read flags

<ResponseField name="--tail" type="int" default="100">
Number of historical lines to replay before live output. The range is 0 to 5000, and `0` streams live output only. This limit applies per source, so `--tail 5` on a Pod writing both kinds returns up to 5 container lines and up to 5 system lines, not 5 in total.
</ResponseField>
