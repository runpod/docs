...
Create a spot Pod with a bid price per GPU:
...
Spot Pods are [interruptible instances](/pods/pricing#spot-instances) that cost less than on-demand Pods. They can be interrupted with a 5-second warning when capacity is needed.
...
<ResponseField name="--bid-per-gpu" type="float">
Bid price per GPU in dollars for creating a [spot Pod](/pods/pricing#spot-instances). GPU Pods only.
</ResponseField>
