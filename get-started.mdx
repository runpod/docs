...
<Steps>
  <Step title="Configure your workload">
    ...
    - **Options**: when you select an [official Runpod template](/pods/templates/overview), two extra options appear. These options are not shown for community templates.
      ...
      - **SSH terminal access**: enables SSH into the Pod. If enabled, paste your SSH public key in the field that appears. [Learn how to create an SSH key](/get-started/credentials#ssh-public-keys).
  </Step>
  ...
  <Step title="Configure storage">
    ...
    **Persistent storage**
    ...
    - **Global volume**: permanent, region-independent storage that any Pod can mount, regardless of data center. Capacity grows with the data you store. See [Global volumes](/storage/globalvolume).
...
<Note>
You'll need an [SSH key added to your account](/get-started/credentials#ssh-public-keys) for this to work. You can run `runpodctl doctor` to set this up automatically.
</Note>
...
<CardGroup cols={2}>
  <Card title="Manage credentials" icon="key" horizontal href="/get-started/credentials">
