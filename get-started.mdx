...
<Steps>
  ...
  <Step title="Configure storage">
    ...
    **Persistent storage**
    ...
    Each type mounts at `/workspace` by default. You can change the path with a template override, or the template itself can set a different default. If you attach a network volume and a global volume and leave both paths at their defaults, the global volume moves to `/workspace-global` and the network volume mounts at `/workspace`. Two volumes cannot share a mount path.
