...
> **Note:** The `stockStatus` field can be especially useful. For example, if `stockStatus` is `"Low"`, it means the GPU is nearly unavailable or may not be available for new pods.
