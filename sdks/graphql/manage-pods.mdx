...
To pull images from a private container registry, include `containerRegistryAuthId` in your mutation. This ID references saved registry credentials from your [Runpod settings](https://www.runpod.io/console/user/settings).
...
curl --request POST \
  ...
  --url 'https://api.runpod.io/graphql?api_key=${YOUR_API_KEY}' \
  --data '{
    "query": "mutation { podFindAndDeployOnDemand( input: { cloudType: ALL, gpuCount: 1, volumeInGb: 40, containerDiskInGb: 40, gpuTypeId: \"NVIDIA RTX A6000\", name: \"Private Image Pod\", imageName: \"my-registry.io/my-private-image:latest\", containerRegistryAuthId: \"YOUR_REGISTRY_AUTH_ID\" } ) { id imageName machineId } }"
  }'
```
</Tab>
...
mutation {
  podFindAndDeployOnDemand(
    input: {
      ...
      imageName: "my-registry.io/my-private-image:latest"
...
<Note>
Container registry authentication applies only to GPU Pods. CPU Pods don't support this parameter.
</Note>
