---
title: "Deploy ComfyUI with Serverless and generate images"
sidebarTitle: "Deploy ComfyUI with Serverless"
description: "Learn how to deploy a Serverless endpoint running ComfyUI from the Runpod Hub and use it to generate images with FLUX.1-dev-fp8."
tag: "NEW"
---
...
- How to deploy a ComfyUI Serverless endpoint using the [Runpod Hub](https://docs.runpod.io/hub/overview).
...
2. In the **Serverless Repos** tab, use the search bar to find the **ComfyUI** listing. Click the listing to expand it.
3. Click **Deploy [VERSION_NUMBER]**, then click **Create Endpoint** to confirm. This creates a fully configured endpoint with the FLUX.1-dev-fp8 model pre-installed and appropriate GPU settings for running ComfyUI workflows.

<Note>
The ComfyUI Hub listing comes with the FLUX.1-dev-fp8 model pre-installed and works only with this model when deployed from the Hub. If you need different models, you'll need to deploy using custom Docker images.
</Note>
...
Create a file called `comfyui_workflow.json` with the following FLUX.1-dev-fp8 workflow:
...
def decode_comfyui_json_and_save_image(json_filepath, output_filename="comfyui_generated_image.png"):
    ...
    try:
        ...
        base64_url = data.get("output", {}).get("message")

        if not base64_url:
            print("Error: 'message' not found in the JSON output.")
            return
...
Congratulations! You've successfully used Runpod's Serverless platform to generate an AI image using ComfyUI with the FLUX.1-dev-fp8 model. You now understand the complete workflow of submitting ComfyUI jobs, monitoring their progress, and retrieving results.
...
- **Node connections**: Arrays like `["30", 1]` reference output 1 from node 30.
- **Model loading**: Node 30 loads the checkpoint file.
- **Text encoding**: Nodes 6 and 33 encode positive and negative prompts.
- **Sampling**: Node 31 performs the actual image generation.
- **Image output**: Nodes 8, 9, and 40 handle image decoding and saving.

You can create custom workflows by modifying node parameters or using the ComfyUI interface to export workflow JSON.

## Next steps

Now that you've learned how to generate images with ComfyUI on Serverless, consider exploring these advanced topics:
...
- Try deploying your own [custom worker](/serverless/workers/custom-worker) with different ComfyUI models or custom nodes.
- Learn about [network volumes](/serverless/storage/network-volumes) for storing custom models and workflows.