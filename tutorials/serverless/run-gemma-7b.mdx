...
import { ServerlessTooltip, EndpointTooltip, WorkerTooltip, ColdStartTooltip } from "/snippets/tooltips.jsx";

This tutorial walks you through deploying a <ServerlessTooltip /> <EndpointTooltip /> with Google's Gemma 3 model using the vLLM worker. You'll deploy the `gemma-3-1b-it` instruction-tuned variant, a lightweight model that runs efficiently on a variety of GPUs.
...
<Tip>
The first request may take longer (30-60 seconds) due to <ColdStartTooltip /> as the endpoint loads the model into GPU memory. Subsequent requests complete in just a few seconds.
</Tip>
...
- Build a custom <WorkerTooltip /> for more specialized use cases.
