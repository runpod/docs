---
title: "Generate your first image with Serverless and Stable Diffusion XL"
sidebarTitle: "Generate your first image with Serverless"
description: "Learn how to deploy a Serverless endpoint running Stable Diffusion XL from the Runpod Hub and use it to generate images."
---


In this tutorial, you will learn how to deploy a Serverless endpoint running Stable Diffusion XL on Runpod, submit image generation jobs, monitor their progress, and decode the resulting images.

[Runpod's Serverless platform](/serverless/overview) allows you to run AI/ML models in the cloud without managing infrastructure, automatically scaling resources as needed. Stable Diffusion XL is a powerful AI model that generates high-quality images from text prompts.
...
- How to generate an image using Stable Diffusion XL.
...
2. In the **Serverless Repos** tab, use the search bar or browser to find the **Stable Diffusion XL** listing. Click the listing to expand it.

3. Click **Deploy [VERSION_NUMBER]**, then click **Create Endpoint** to confirm. This creates a fully configured endpoint with appropriate GPU and worker settings for running Stable Diffusion XL.

4. On the endpoint page, make a note of the **Endpoint ID** (e.g. ) and **API Key**. You'll need these values to submit jobs and retrieve results.
...
4. Configure your endpoint settings by selecting an appropriate GPU type. For this tutorial, choose a GPU with at least 16GB of VRAM, such as an RTX 4090 or A100. Set your worker configuration with a minimum of 0 workers and a maximum of 3 workers to allow for auto-scaling.

5. Click **Deploy** to begin the setup process.
