...
import { ServerlessTooltip, CachedModelsTooltip, ColdStartTooltip, HandlerFunctionTooltip } from "/snippets/tooltips.jsx";

This tutorial demonstrates how to build a custom <ServerlessTooltip />
 worker that leverages Runpod's <CachedModelsTooltip /> to serve the Phi-3 language model. You'll learn how to create a handler function that locates and loads cached models in offline mode, which can significantly reduce costs and <ColdStartTooltip /> times.

## What you'll learn

- How to configure a Serverless endpoint with a cached model.
- How to programmatically locate a cached model in your <HandlerFunctionTooltip />.
