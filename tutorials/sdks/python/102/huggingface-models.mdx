...
Loading the model in a function ensures that the model is only loaded once when the <WorkerTooltip /> starts, optimizing the performance of our application. Add the following code to your `sentiment_analysis.py` file:
...
### Define the <HandlerFunctionTooltip />
...
### Start the Serverless <WorkerTooltip />

To run our sentiment analysis function as a Serverless <WorkerTooltip />, we need to start the worker using Runpod's SDK. Add the following line at the end of your `sentiment_analysis.py` file:

```python sentiment_analysis.py
runpod.serverless.start({"handler": sentiment_analysis_handler})
```

This command starts the Serverless worker and specifies `sentiment_analysis_handler` as the <HandlerFunctionTooltip /> for incoming requests.
