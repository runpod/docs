...
<Note>
When deploying queue-based functions with `flash deploy`, each function must have its own unique resource configuration. If your script has multiple functions sharing the same config (like `generate_text` and `chat` in this tutorial), create separate configs for each function when converting to a Flash app. See [understanding endpoint architecture](/flash/apps/deploy-apps#understanding-endpoint-architecture) for details.
</Note>

### Optimize performance

- Use [network volumes](/storage/network-volumes) to cache models across workers.
...
- [Flash resource configuration](/flash/configuration/overview).
