...
<Note>
The `aws:PrincipalArn` value (`arn:aws:iam::418399314813:role/prod-us-east-1-deployment-role`) is Runpod's fixed AWS deployment role — not something you create or manage. `418399314813` is Runpod's AWS account ID, and this ARN is the same for all Runpod users. Do not modify it. Its permissions are intentionally limited to pulling images only, so Runpod cannot access any other part of your AWS account.
</Note>
