...
This tutorial shows how to deploy <PodTooltip />s using container images stored in private AWS Elastic Container Registry (ECR) repositories. You configure cross-account IAM delegation so Runpod can pull images on your behalf, with no credential sharing required and no expiring tokens to manage.
...
- Confirm the ECR repository in the <PodTooltip /> or template matches the repository registered in Step 3. The tag is not checked at pull time. Any tag from a registered repository works.
