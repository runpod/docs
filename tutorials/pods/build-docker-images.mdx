...
Replace `USERNAME_PLACEHOLDER` in `BUILD.bazel` with your Docker Hub username. For example, if your username is `myusername`:

```bash
sed 's/USERNAME_PLACEHOLDER/myusername/g' BUILD.bazel > BUILD.bazel.tmp && mv BUILD.bazel.tmp BUILD.bazel
```
...
```
repository = "index.docker.io/myusername/custom_image",
```
