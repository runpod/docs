...
This tutorial uses the [SDXL-Turbo](https://huggingface.co/stabilityai/sdxl-turbo) model and a matching template, but you can adapt these instructions for any model/template combination you wish to use.
...
<Steps>
  <Step title="Deploy a Pod from the ComfyUI template">
    Go to the [Better Comfy Slim](https://console.runpod.io/explore/cndsag8ob0) template page in the Runpod console and click **Deploy**.
    
    <Note>
    There are several ComfyUI templates available in the Runpod console. We're using this one because it's extremely lightweight, allowing you to select which models and nodes to install after deployment, and it comes with the ComfyUI Manager plugin pre-installed.
    </Note>
  </Step>
...
Follow these steps to load a workflow template:
...
<Tip>
When you're just getting started with ComfyUI, always use the workflow that was created for the specific model you intend to use. You usually can't just switch the Load Checkpoint node from one model to another and expect it to work optimally.

For example, if you load a workflow created for Flux Dev and try to use it with an SDXL-Turbo model checkpoint, the workflow may run, but with poor speed or image quality.
</Tip>

## Step 4: Install the SDXL-Turbo model checkpoint

As soon as you load the workflow, you'll see a popup labeled **Missing Models**. This is because the Pod template we deployed doesn't come pre-installed with any models, so you'll need to install them now.
...
<Steps>
  ...
  <Step title="Refresh your browser">
    Before you can use the model, you'll need to refresh the browser tab where the ComfyUI interface is running.
  </Step>
...
### Try a different workflow template

Use the template browser from [Step 4](#step-4-load-a-workflow-template) to test out new models and find a workflow that suits your needs.
...
Don't forget to install any missing models using the model manager. If you need a model that isn't available in the model manager, you can download it from the web and use the [Runpod CLI](/runpodctl/transfer-files) to transfer the model files directly into your Pod's `/workspace/madapps/ComfyUI/models` directory.
