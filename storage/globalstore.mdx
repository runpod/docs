...
The mount path depends on whether a network volume is also attached:

- **Global volume only:** mounts at `/workspace`.
- **Global volume and network volume together:** global volume mounts at `/workspace-global`, network volume mounts at `/workspace`.

You can verify the volume is mounted from the Pod's **Console** tab:
...
If you have data on a network volume that you want to move to a global volume, you can use `rsync`. When both volumes are attached, the network volume is at `/workspace` and the global volume is at `/workspace-global`.

1. Deploy a Pod with both your network volume and global volume attached.
