...
Global volume cost has two parts: storage and request activity. Storage is billed per GB per month based on how much data you keep on the volume. The [storage comparison](/pods/storage/types#comparison) table shows the current per-GB storage rate for each storage type, including global volumes.

Because your access pattern affects cost as much as how much data you store, read-mostly workloads that reuse cached data stay inexpensive, while workloads that perform many write operations, or that repeat metadata operations at high volume, cost more.

<Note>
Global volume pricing is still evolving during beta and may change.
</Note>
