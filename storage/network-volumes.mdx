...
<Note>
Each network volume lives in a single datacenter and cannot be moved. Any Pod, worker, or Instant Cluster that attaches the volume must run in that same datacenter, which limits the GPU inventory available to those workloads. To spread across datacenters, create a separate volume in each one and copy data between them manually (see [Migrate files between volumes](#migrate-files-between-volumes)).
</Note>
