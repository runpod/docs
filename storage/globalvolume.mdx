...
1. Go to [Serverless](https://www.console.runpod.io/serverless/user/endpoints) and click **+ New Endpoint**.
...
Attaching a volume doesn't copy any existing data, so upload your model files and other assets to the global volume before you attach it. Changing the attached volume is a configuration change, so it triggers a new release and your workers redeploy to pick it up.

An endpoint can attach at most one global volume. It can't use a global volume and a network volume at the same time, but it can attach [multiple network volumes](/storage/network-volumes#network-volumes-for-serverless) on their own. To switch an endpoint from network volumes to a global volume, detach the network volumes first. To switch back, remove the global volume first.
