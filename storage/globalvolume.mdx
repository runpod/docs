...
Changing the attached volume is a configuration change, so it triggers a new release and your workers redeploy to pick it up.

An endpoint supports one volume at a time. You can attach either a global volume or a network volume, but not both. To attach a network volume instead, remove the global volume first.

### Attach a global volume with the REST API

You can also attach and detach an endpoint's global volume through the [Runpod REST API v2](/api-reference-v2/overview). The endpoint create, update, and read operations accept a `globalVolumes` field: an array of volume IDs that holds at most one volume. It mirrors the existing `networkVolumes` field.
...
The REST API can attach global volumes to Serverless endpoints only. To attach a global volume to a Pod, use the console (see [Attach a global volume to a Pod](#attach-a-global-volume-to-a-pod)).

For the full request schemas, see [Create a Serverless endpoint](/api-reference-v2/serverless/create-a-serverless-endpoint) and [Update a Serverless endpoint](/api-reference-v2/serverless/update-a-serverless-endpoint).
