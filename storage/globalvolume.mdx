...
Global volume cost has two parts: storage and request activity. Storage is billed per GB per month based on how much data you keep on the volume. Request activity covers the reads and writes your workload performs against the volume.

Because a global volume stores your data as objects in cloud storage rather than on a local disk, filesystem operations do not map one-to-one to billable storage requests. A single filesystem command can translate into a variable number of underlying storage requests, depending on what the command does. Caching softens this. The caching layer between your Pod and the volume (FUSE, Filesystem in Userspace) caches some operations, such as the directory listings returned by `ls`, for up to a minute. Repeating them does not generate a new request each time.

As a result, your access pattern affects cost as much as how much data you store. Read-mostly workloads that reuse cached data stay inexpensive, while workloads that perform many write operations, or that repeat metadata operations at high volume, cost more.

The [storage comparison](/pods/storage/types#comparison) table shows the current per-GB storage rate for each storage type, including global volumes.
