...
**Region** stays set to **Any region** when a global volume is the only volume attached, so you can choose from GPUs in any data center. Adding a network volume restricts the Pod to that volume's data center and narrows the available GPUs.
