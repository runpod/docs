...
Every Pod has an hourly cost based on [GPU type](/references/gpu-types). Your Runpod credits are charged every minute the Pod is running.

<Important>
To ensure uninterrupted workloads, maintain an account balance covering at least one hour of your current runtime. Runpod automatically stops Pods when your balance drops below the projected cost of your active workloads. Pods with attached network volumes are deleted (not just stopped) when this threshold is reached. Enable [Auto-pay](/accounts-billing/billing#auto-pay) and set up [low-balance notifications](https://www.console.runpod.io/user/settings) with at least two hours of runway to avoid unexpected interruptions.
</Important>

You must have at least one hour's worth of runtime in your balance to rent a Pod. If your balance is insufficient, deposit additional funds or select a lower-cost GPU configuration.
...
Storage is charged per minute. You are not charged for storage if the host <MachineTooltip /> is down or unavailable from the public internet.
