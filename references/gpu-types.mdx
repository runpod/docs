...
The table below lists the GPU pools that you can use when defining requirements for repositories published to the [RunPod Hub](/hub/publishing-guide#runpod-configuration), or when specifying GPU requirements for an endpoint with the [RunPod GraphQL API](https://graphql-spec.runpod.io/).

These pools define which GPUs are available to workers on an endpoint after deployment.
