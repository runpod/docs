...
### What's the difference between Pod logs and Serverless logs?

**Pod Logs** are temporary and only exist on the server where the Pod is running. When a Pod is stopped, all logs are removed. These logs have no throttling but do not persist for long.

**Serverless Logs** are automatically streamed to RunPod's centralized storage system and persist for at least 90 days. They are accessible through the Logs tab in your endpoint dashboard. These logs may have throttling applied if workers generate excessive output to prevent system overload.

For permanent log storage beyond 90 days, you can write logs to [network volumes](/serverless/storage/network-volumes) within your handler functions. [Learn more about serverless logging](/serverless/logs).
